...
class Decoder(nn.Module):
    def __init__(
        ...
        # Build hnet
        hnet_dims = [latent_size + subnet_pts_in] + subnet_dims + [latent_size]
    ...
    def forward(self, net_input, use_net=None):
        ...
        if use_net is None:
            ...
            # Feed forward hnet
            x = ff_hnet(subet_input)
            c_code = x
            c_x = ff_fnet(torch.cat([x, pts], 1))

            # Finally, feed forward Bnet
            b_x = ff_fnet(net_input)

            if self.do_code_regularization:
                return (c_x, b_x, r_x, r_code, c_code)
            return (c_x, b_x, r_x)
        elif use_net == 0:
            c_code = ff_hnet(subet_input)
            return ff_fnet(torch.cat([c_code, pts], 1))
        elif use_net == 1:
